...
func main() {
	...
	r.GET("/demo", func(c *gin.Context) {
		c.JSON(200, gin.H{
			...
			"description": "This endpoint demonstrates the moderation middleware functionality",
			"endpoints": []string{
				"POST /public/feedback - Public feedback (no auth, no moderation)",
				"POST /api/posts - Create post (auth + moderation required)",
				"POST /api/comments - Create comment (auth + moderation required)",
				"PUT /api/profile - Update profile (auth + moderation required)",
				"GET /api/posts - List posts (auth required, no moderation)",
				"GET /api/profile - Get profile (auth required, no moderation)",
			},
		})
	})
